...
'''This script fetches weather data using the Open-Meteo API for a specified city. It provides functions to retrieve 
both daily and hourly weather data, preprocess it, and save it to CSV files. Additionally, it includes functions to 
determine whether weather data is available for a given city and to find the latest date with weather data'''
...
def weather_daily(city, start_date, end_date):
    ...
    daily_et0_fao_evapotranspiration = daily.Variables(4).ValuesAsNumpy()
    ...
    daily_data = {
        ...
        "evapotranspiration": daily_et0_fao_evapotranspiration
    }
...
def get_weather_data(city, save, type):
    ...
    if is_available:
        ...
        if type == 'daily':
            ...
            start_date = (pd.to_datetime(end_date) - pd.DateOffset(years=15)).strftime('%Y-%m-%d')
            ...
            # Ensure column names match for preprocessing
            df = data_preprocessing(df, ["temperature", "relative_humidity_2m", "PRECIPITATION", "surface_pressure",
                                         "wind_speed"])
            end_date = end
